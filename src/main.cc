...
void main()
{
	...
	triangle = vec3(vertex_position.x, vertex_position.y, vertex_position.z);
}
)zzz";
...
void main()
{
	...
	face_normal = normalize(cross(triangle[1]-triangle[0],triangle[2]-triangle[0]));
	for (n = 0; n < gl_in.length(); n++) {
		light_direction = vs_light_direction[n];
		gl_Position = projection * gl_in[n].gl_Position;
		world_position = triangle[n];
		EmitVertex();
	}
	EndPrimitive();
}
...
void main()
{
	vec4 color = vec4(abs(face_normal), 1.0);
        ...
        std::vector<glm::uvec3>& indices)
{
	float a = -500.0f;
	float b = 500.0f;
	vertices.push_back(glm::vec4(a, -2.0, a, 1.0f));
	vertices.push_back(glm::vec4(a, -2.0, b, 1.0f));
	vertices.push_back(glm::vec4(b, -2.0, a, 1.0f));
	//vertices.push_back(glm::vec4(0.5f, 0.5f, -0.5f, 1.0f));
	indices.push_back(glm::uvec3(0, 1, 2));
	vertices.push_back(glm::vec4(b, -2.0, b, 1.0f));
	vertices.push_back(glm::vec4(a, -2.0, b, 1.0f));
	vertices.push_back(glm::vec4(b, -2.0, a, 1.0f));
	//vertices.push_back(glm::vec4(0.5f, 0.5f, -0.5f, 1.0f));
            ...
            int mods)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		// FIXME: WASD
		if (!g_camera.mode())
			g_camera.zoom(glm::dvec2(0.0, -1.0));
		else 
			g_camera.zoom(-1.0);
...
{
	glm::dvec2 mouse_direction = glm::normalize(glm::dvec2(mouse_x - old_mouse_x, mouse_y - old_mouse_y)); 
	if (!g_mouse_pressed)
		return;
	if (g_current_button == GLFW_MOUSE_BUTTON_LEFT) {
		// FIXME: left drag
		g_camera.rotate(mouse_direction);
	} else if (g_current_button == GLFW_MOUSE_BUTTON_RIGHT) {
		// FIXME: right drag
		g_camera.zoom(mouse_direction);
...
int main(int argc, char* argv[])
{
	...
	g_menger->set_clean();
	   //g_menger->generate_subcube(obj_vertices, obj_faces, glm::vec3(0.0f, 0.0f, 0.0f), 0.5f);
